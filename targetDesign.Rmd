...
Export guide list:
```{r}
write.xlsx(targetGDO, file = paste0(wd, "//output//targetGDOi30.xlsx"))

# old <- read.xlsx(paste0(wd, "//output/targetGDOi30.xlsx"))
# test <- anti_join(targetGDO, old, by = "sgRNA.Sequence") # Identical
...
Add/drop of GDOs is based on targetGDOi30.xlsx.
...
targetGDO <- read.xlsx(paste0(wd,"//output//targetGDOi30.xlsx"))
```


Regions for rs1870138|rs7080009 and rs1870137 are ~50bp away from each other. Let's combine them.
We can reuse the guides from the Pilot Screen; these are already included in the library.
We then rename the region to reflect the merging of the two regions.
...
```{r}
write.xlsx(targetGDO, file = paste0(wd, "//output//targetGDOi30_wPilot.xlsx"))

# old <- read.xlsx(paste0(wd,"//output//targetGDOi30_wPilot.xlsx"))
# test <- anti_join(targetGDO, old, by = "sgRNA.Sequence")
...
# Post-Cellecta QC
Remove guides that did not pass QC + pick new guides to replace them.
```{r}
# targetGDO <- read.xlsx(paste0(wd, "//output//oldTargetGDOi30_remBadReg.xlsx"))

removeGDO <- c("GGGGGAAGTAGTCAGAGATC","GGGGGAAGAAGACCCTGCAG") # rs1140239_1 and rs785143_3 respectively.
targetGDO <- subset(targetGDO, !(sgRNA.Sequence %in% removeGDO))

guidePool <- read.xlsx(paste0(wd, "//preprocessed//200bp_i30.xlsx"))
guidePool <- subset(guidePool, (SNP == "rs1140239"))
guidePool <- filterGDO(guidePool, minOnScore = 0.2)

targetGDO <- bind_rows(targetGDO, subset(guidePool, sgRNA.Sequence=="GGAAGTAGTCAGAGATCTGG"))

guidePool <- read.xlsx(paste0(wd, "//preprocessed//200bp_i30.xlsx"))
guidePool <- subset(guidePool, (SNP == "rs785143"))
guidePool <- filterGDO(guidePool, minOnScore = 0.2)

targetGDO <- bind_rows(targetGDO, subset(guidePool, sgRNA.Sequence=="TGTCTAAAAGGGAACCACAG"))
```


```{r}
write.xlsx(targetGDO, file = paste0(wd, "//output//targetGDOi30_remBadReg.xlsx"))

# old <- read.xlsx(paste0(wd, "//output//oldTargetGDOi30_remBadReg.xlsx"))
# test <- anti_join(targetGDO, old, by = "sgRNA.Sequence")
```
