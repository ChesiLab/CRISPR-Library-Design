...
target <- read.xlsx(paste0(wd,"//output//finalTargetGDOi30.xlsx"))
promoter <- read.xlsx(paste0(wd,"//output//finalPromoterGDOa30.xlsx"))
ntc <- read.xlsx(paste0(wd,"//input//NTC_list.xlsx"))
...
```{r}
targetUCSC <- rbind(promFinal, targetFinal) %>%
...
regions200 <- regions200 %>%
  mutate(UCSC = paste0("chr",chr,":",start,"-",end)) %>%
  select(SNP, UCSC)

write.xlsx(regions200, paste0(wd,"//postprocessed//regionsUCSC.xlsx"))
```
...
# Export Guide List for Ordering
```{r}
orderList <- rbind(
  select(promFinal, ID, sgRNA.Sequence),
  select(targetFinal, ID, sgRNA.Sequence),
  select(ntcFinal, ID, sgRNA.Sequence)
)

names(orderList) <- c("Guide_ID", "Sequence")

write.xlsx(orderList, paste0(wd,"//postprocessed//orderList.xlsx"))
```

```{r}
length(unique(orderList$Sequence))

reg <- data.frame(unique(target$SNP))
names(reg) <- "SNP"

anti_join(reg, select(regions200, SNP))

regions300 <- read.xlsx(paste0(wd,"//preprocessed//regions300.xlsx"))
overlap <- findOverlap(target, regions300,
                          "chr", "start", "end",
                          "chr", "start", "end")

regions500 <- read.xlsx(paste0(wd,"//preprocessed//regions500.xlsx"))
overlap <- findOverlap(target, regions500,
                          "chr", "start", "end",
                          "chr", "start", "end")

qcSimple(target)
strand <- qcStrand(target)

selfOverlap <- findSelfOverlap(target, "chr", "start", "end", selfBool=TRUE)

targetex <- na.omit(target)
loverlap <- findSelfOverlap(targetex, "chr", "startL", "endL", selfBool = FALSE)
```
