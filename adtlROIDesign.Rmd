...
```
## Assemble Library
adtlGDO library (12 guides for del region)
```{r}
...
reg300GDO <- assembleLibrary(3, fileList, regionList, dirGDO, dirRegion,
                          onTargetFilter = 0.2, pickByStrand = TRUE, overlapLeniency=2)

uniqueGDO <- unique(reg300GDO)
nrow(uniqueGDO) == nrow(reg300GDO)

overlap <- findSelfOverlap(reg300GDO, "chr", "start","end", selfBool=TRUE)
overlapLenient <- findSelfOverlap(reg300GDO, "chr", "startL","endL", selfBool=TRUE)

combinedReg <- c("rs1317708|rs4989024|rs874424", "rs2287322|rs8077638|rs34962442|rs62090051")

combinedGDO <- subset(reg300GDO, SNP %in% combinedReg)
```


```{r}
targetGDO <- read.xlsx(paste0(wd,"//output//targetGDOi30_remBadReg.xlsx"))
# oldTarget <- read.xlsx(paste0(wd,"//output//oldTargetGDOi30_remBadReg.xlsx"))

# test <- anti_join(targetGDO, oldTarget, by = "sgRNA.Sequence")
# test <- data.frame(unique(targetGDO$sgRNA.Sequence))
# test <- data.frame(table(targetGDO$SNP))
...
```{r}
# Compare to old list before Cellecta QC:
# oldTarget <- read.xlsx(paste0(wd,"//output//finalTargetGDOi30.xlsx"))
# test <- anti_join(targetGDO, oldTarget, by="sgRNA.Sequence")
# test <- anti_join(oldTarget, targetGDO, by="sgRNA.Sequence")
```
